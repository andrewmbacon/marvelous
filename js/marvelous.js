 ...
 * Build: http://modernizr.com/download/#-boxshadow-csstransforms-shiv-cssclasses-testprop-testallprops-domprefixes-load
 */
...
// Utilities
function duplicate(from, num, to){
	var target = $(to)
	for (i=0; i<num; i++){
		var source = $(from).clone()
...
duplicate('.event:first-child', 39, '#skew1')
...
$( window ).load(function() {
	...
	$('#events').each(function(){
		...
		var skewWidth = $('.event').width();
		var skewHeight = $('.event').height();
		
		
		var skewSpacing =  parseInt($('.event').css('margin-right'));
		var skewAdjust = (skewWidth+skewSpacing) * Math.tan(skewRadians);

		var currentAdjust = 0;
	
		var skewCellsN = $(this).children('.event').length;
		...
		$('.angle-cell').each(function(){
			...
			$(this).find('img').each(function(){
				...
				var targetW = $('.angle-img').width() +1;
				var targetH = $('.angle-img').height() + skewAdjust + 1;
